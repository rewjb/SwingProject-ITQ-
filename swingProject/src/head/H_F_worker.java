 ...
 * wHn
 * 2018-12-13 수정완료
 * 가맹점 아이디 생성에 관련된 동작을 하는 메서드들을 모아놓은 클래스
 */
...
/*
 * 현재 DB에 있는 마지막 아이디를 가져오는 메서드
 * 가맹점 동작하는 클래스에 마지막 아이디 값을 받아오는 변수가 있음(String id)
 * DAO의 select all을 실행하면서 그 순서의 id를 매번 넣어줌(String input)
 * 반복문이 한번 실행될때마다 받아져있는 마지막 아이디와 현재 아이디를 비교해서 
 * 마지막 순서의 아이디를 반환해주는 메서드
 */
	String findLastId(String id, String input) {
		...
		if (id == null || input == null) {
			/*둘중 하나이상이 null일경우에는 비교하지 않고 input 값을 반환해준다.
			 * 가맹점 동작하는 클래스에서 이 메서드의 반환값을 받는 변수는 id임.
			 * 둘다 null일경우 : 둘다 null이어서 그냥 null을 반환
			 * id가 null일 경우 : input된 값을 반환해서 id에 넣어준다.
			 * input이 null일 경우 : DB에 데이터가 없다는 뜻이므로 null을 반환해준다.
			 */
		} else {
